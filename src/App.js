...
const getTemplateMeta = (id, options = DEFAULT_TEMPLATE_OPTIONS) =>
...
export default function App() {
  ...
  const [templateId, setTemplateId] = useState(DEFAULT_TEMPLATE_OPTIONS[0].id);
  ...
  useEffect(() => {
    ...
    async function loadTemplates() {
      try {
        ...
        const withCustom = [...liveTemplates, DEFAULT_TEMPLATE_OPTIONS.find((t) => t.id === "custom")];

        if (!ignore && withCustom.length > 1) {
          setTemplateOptions(withCustom);
          setTemplateId(liveTemplates[0]?.id || DEFAULT_TEMPLATE_OPTIONS[0].id);
        } else if (!ignore) {
          ...
          setTemplateId(DEFAULT_TEMPLATE_OPTIONS[0].id);
        }
      } catch (err) {
        console.warn("Falling back to predefined template list", err);
        if (!ignore) setTemplateOptions(DEFAULT_TEMPLATE_OPTIONS);
      }
    }
